...
最近重新思考設計 API 要用什麼方式時，內心劇場演著「REST 還是 action 導向」劇本。REST 純粹主義者搬出 Fielding 2000 年的博士論文，說 `POST /orders` 才是正義；商業派搬出 Netflix 的 use-case 主張，說資源導向在大規模下會逼客戶端用一堆請求才能完成一件「事」，`POST /checkout` 才是正義。
